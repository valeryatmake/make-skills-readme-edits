...
- **Connection type ≠ app name for filtering.** When calling `connections_list` with a `type` filter, use the `accountName` value (e.g., `"google"` for Google Sheets/Calendar/Drive, not `"google-sheets"`). Check the `accountName` field in existing connection objects to determine the correct filter value.
